...
void Test_BLINKER_APP_Main(void)
{
    ...
    /*
     * BLINKER_APP_Main does not return a value,
     ...
     * The first is for BLINKER_APP_Init().  As this is in the same
     ...
     * the BLINKER_APP_Init() failure path was taken.
     */
    BLINKER_APP_Main();
     ...
     */
    UtAssert_UINT32_EQ(BLINKER_APP_Data.RunStatus, CFE_ES_RunStatus_APP_ERROR);
    ...
    UT_CHECKEVENT_SETUP(&EventTest, BLINKER_APP_PIPE_ERR_EID, "BLINKER APP: SB Pipe Read Error, App Will Exit");
...
void Test_BLINKER_APP_Init(void)
{
    /*
     * Test Case For:
    ...
    UtAssert_INT32_EQ(BLINKER_APP_Init(), CFE_SUCCESS);
     ...
     */
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UtAssert_INT32_EQ(BLINKER_APP_Init(), CFE_EVS_INVALID_PARAMETER);
    UtAssert_STUB_COUNT(CFE_ES_WriteToSysLog, 1);

    UT_SetDeferredRetcode(UT_KEY(CFE_SB_CreatePipe), 1, CFE_SB_BAD_ARGUMENT);
    UtAssert_INT32_EQ(BLINKER_APP_Init(), CFE_SB_BAD_ARGUMENT);
    ...
    UtAssert_INT32_EQ(BLINKER_APP_Init(), CFE_TBL_ERR_INVALID_OPTIONS);
...
{
    ADD_TEST(BLINKER_APP_Main);
    ADD_TEST(BLINKER_APP_Init);
}
